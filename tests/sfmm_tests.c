...
Test(sf_memsuite_student, malloc_exactly_4_pages, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_malloc(16384);
	cr_assert(((sf_header*)seg_free_list[3].head)->block_size << 4== (4096*4), "List has all 4 pages");

}

Test(sf_memsuite_student, malloc_1_page_realoc_less_than_1_page, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *ptr = sf_malloc(4064);
	sf_realloc(ptr, 4048);

	cr_assert(((sf_header*)seg_free_list[0].head)->block_size << 4== (32), "List has Min Block");
}

Test(sf_memsuite_student, multiple_malloc_and_free, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *ptr[32];
	...
	for(int i = 0;i<4;i++){
		cr_assert_null(seg_free_list[i].head, "All Lists are Empty");
	}
}
Test(sf_memsuite_student, 4_page_malloc_than_2times_realloc_smaller, .init = sf_mem_init, .fini = sf_mem_fini) {
	void* x = sf_malloc(16368);
	void *y = sf_realloc(x, 1);
	sf_realloc(y, 0);

	cr_assert(seg_free_list[3].head->header.block_size << 4 == 16384);
}
Test(sf_memsuite_student, Big_malloc_small_realloc_free, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(16000);
